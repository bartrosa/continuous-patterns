...
- **`gravity_sweep_<YYYYMMDD_HHMMSS>/`** — Experiment 4 (`configs/agate_ch/gravity/`): subfolders **`alpha_0_00`** … **`alpha_0_20`** and **`manifest.json`**. Run **`plot_gravity_sweep --manifest …/manifest.json`** to write **`gravity_comparison.png`** and **`gravity_vertical_profiles.png`** into **`results/agate_ch/`** (override with **`--output-dir`**).
