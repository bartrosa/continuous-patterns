...
def print_ring_mask_sanity(
    ...
    *,
    ...
    implied_ring_thickness = ring_area_physical / cavity_perimeter if cavity_perimeter > 0 else 0.0
    ...
    print(
        f"  implied ring thickness (wide): {implied_ring_thickness / dx:.3f} dx={dx:.3f}",
        file=out,
    )
...
def _integrate_flux_diagnosis(
    ...
    silica0 = float(
        total_silica_jnp(state0[0], state0[1], state0[2], geom.chi, geom.dx, prm.rho_m, prm.rho_c)
    )
    _dxdiag = float(geom.dx)
    _odx = float(cfg.get("physical_flux_outer_dx", 3.0))
    _idx = float(cfg.get("physical_flux_inner_dx", 5.0))
    _r_outer_disk, _ = radial_shell_widths_from_dx(_dxdiag, R, outer_dx=_odx, inner_dx=_idx)
    ...
    c_disk_0 = dissolved_silica_mass_inside_disk_numpy(c0_np_init, L=L, r_disk=_r_outer_disk)
    c_disk_1 = dissolved_silica_mass_inside_disk_numpy(c1_np_fin, L=L, r_disk=_r_outer_disk)
    ...
    for tgt in milestones:
        ...
        s_c_inside = dissolved_silica_mass_inside_disk_numpy(c_np, L=L, r_disk=_r_outer_disk)
...
def integrate_chunks(
    cfg: dict[str, Any],
    chunk_size: int,
    on_snapshot: (Callable[[int, jnp.ndarray, jnp.ndarray, jnp.ndarray], None] | None) = None,
) -> tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, dict[str, Any]]:
    ...
    def _silica_total(st: tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]) -> float:
        return float(total_silica_jnp(st[0], st[1], st[2], geom.chi, geom.dx, prm.rho_m, prm.rho_c))
    ...
    def _print_diag(n: int, st: tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]) -> None:
        ...
        print(
            f"step {n}: |phi_m|>1.05 fraction = {fm:.4f}, |phi_c|>1.05 fraction = {fc:.4f}",
            file=sys.stderr,
        )
    ...
    flux_sample_every = max(1, int(round(flux_sample_dt / dt))) if flux_sample_dt > 0 else 0
...
def simulate(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    def mass_of(st: tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]) -> float:
        return float(
            jax.device_get(
                total_silica_full_domain_jnp(st[0], st[1], st[2], dx_j, prm.rho_m, prm.rho_c)
            )
        )
