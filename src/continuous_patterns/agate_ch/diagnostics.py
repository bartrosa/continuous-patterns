...
def _sample_right_half_ray(
    phi: np.ndarray, L: float, R: float, theta: float
) -> np.ndarray:
...
def horizontal_centerline(
    phi: np.ndarray, L: float, R: float
) -> tuple[np.ndarray, np.ndarray]:
...
def moganite_chalcedony_anticorr(
...
def silica_mass_inside_disk_numpy(
    ...
    *,
    ...
    mask = rv < r_disk
    rho_total = (
        np.asarray(c_field) + rho_m * np.asarray(phi_m) + rho_c * np.asarray(phi_c)
    )
...
def compute_influx_rate_physical(
    ...
    *,
    ...
    n = int(c_field.shape[0])
    dx = L / n
    r_outer, r_inner = radial_shell_widths_from_dx(
        dx, R, outer_dx=outer_dx, inner_dx=inner_dx
    )
    c_outer = azimuthal_mean_at_radius_numpy(
        c_field, L=L, r_abs=r_outer, n_theta=n_theta
    )
    c_inner = azimuthal_mean_at_radius_numpy(
        c_field, L=L, r_abs=r_inner, n_theta=n_theta
    )
    dr_shell = r_outer - r_inner
...
def integrate_influx_physical_trapz(
    snaps: list[tuple[int, Any, Any, Any]],
    cfg: dict[str, Any],
) -> tuple[np.ndarray, np.ndarray, float]:
    """Trapezoidal ∫ influx_rate dt using saved snapshots (times = step·dt)."""
    dt = float(cfg["dt"])
    L = float(cfg["L"])
    R = float(cfg["R"])
    D_c = float(cfg["D_c"])
    ...
    snaps_sorted = sorted(snaps, key=lambda x: int(x[0]))
    times: list[float] = []
    rates: list[float] = []
    for step, c, _, _ in snaps_sorted:
    ...
    if t_arr.size >= 2:
        influx = float(np.trapezoid(r_arr, t_arr))
    elif t_arr.size == 1:
        influx = float(r_arr[0]) * float(cfg["T"])
    else:
        influx = 0.0
    return t_arr, r_arr, influx


def physical_mass_balance_from_snapshots(
    snaps_full: list[tuple[int, Any, Any, Any]],
    cfg: dict[str, Any],
    *,
    silica_initial: float,
    silica_final: float,
) -> dict[str, Any]:
    """Independent influx integral vs cavity silica; residual = leak indicator."""
    _, _, influx_physical = integrate_influx_physical_trapz(snaps_full, cfg)
    residual = float(silica_final - silica_initial - influx_physical)
    denom = max(
        abs(silica_initial),
        abs(silica_final),
        abs(influx_physical),
    ...
    mb_pct = 100.0 * residual / denom
    return {
        "influx_total_physical": influx_physical,
        "residual_physical": residual,
        "mass_balance_percent_physical": mb_pct,
