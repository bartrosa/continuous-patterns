"""Run Experiment 6: Flamant stress field sweep (four configs).

Writes ``results/agate_ch/stress_sweep_<timestamp>/{stress_off,...}/`` and
``manifest.json`` for :mod:`plot_stress_sweep`.

Example::

    uv run python -m continuous_patterns.agate_ch.run_stress_sweep
    uv run python -m continuous_patterns.agate_ch.run_stress_sweep --no-progress
"""
...
def main(argv: list[str] | None = None) -> None:
    root = _repo_root()
    ap = argparse.ArgumentParser(description="Sequential Experiment 6 stress sweep.")
    ap.add_argument(
        "--no-progress",
        action="store_true",
    ...
    names = ["stress_off", "flamant_B_0_5", "flamant_B_1_0", "flamant_B_2_0"]
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    sweep_root = root / "results" / "agate_ch" / f"stress_sweep_{ts}"
    sweep_root.mkdir(parents=True, exist_ok=True)
    manifest: dict[str, Any] = {"timestamp": ts, "runs": []}
    exe = sys.executable

    print(
        f"Experiment 6 stress sweep\n"
