...
def plot_mass_balance_comparison(
    *,
    ...
    fr_b = np.asarray(surface_flux_budget.get("flux_rates_valid") or [], dtype=np.float64)
    ax_a = axes[0, 0]
    if dt_b.size:
    ...
    if direct_delta == direct_delta and flux_b == flux_b:
        rel_lines.append(f"flux vs Δdirect: {100.0 * (flux_b - direct_delta) / denom_annot:+.2f}%")
    if rel_lines:
        ax_d.text(
            0.02,
            0.98,
