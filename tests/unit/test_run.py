...
def test_run_one_dispatches_agate_ch(tmp_path: Path) -> None:
    ...
    result = run_one(cfg, results_root=tmp_path, chunk_size=50, show_progress=False)
    ...
    assert result.paths.log_file == result.paths.root / "run.log"
    assert result.paths.log_file.is_file()


def test_run_one_writes_log_file(tmp_path: Path) -> None:
    """``run.log`` is created and records simulation start and completion."""
    cfg = _minimal_agate_ch_cfg()
    result = run_one(cfg, results_root=tmp_path, chunk_size=50, show_progress=False)
    assert result.paths is not None
    log_content = result.paths.log_file.read_text(encoding="utf-8")
    assert "Simulation start" in log_content
    assert "Simulation complete" in log_content


def test_run_one_no_write_skips_log_file(tmp_path: Path) -> None:
    """No ``run.log`` under ``tmp_path`` when ``write_artifacts=False``."""
    cfg = _minimal_agate_ch_cfg()
    result = run_one(
        cfg,
...
def test_run_one_unknown_model() -> None:
    ...
    with pytest.raises(ValueError, match="Unknown model"):
        run_one(cfg, write_artifacts=False, chunk_size=10, show_progress=False)
